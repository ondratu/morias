...
def _get(self, c):
    c.execute("""
        SELECT client_id, email, create_date, modify_date, state, items,
            history, data
        FROM eshop_orders WHERE order_id = %d""", self.id)
    row = c.fetchone()
    if not row:
        return None
...
def _add(self, c):
    c.execute("""
        INSERT INTO eshop_orders (client_id, email, create_date,
    ...
    c.execute(
        "SELECT order_id, create_date FROM eshop_orders WHERE rowid = %d",
        c.lastrowid)
