...
def add(self, req):
    ...
    c.execute("""
            INSERT INTO attachments (uploader_id, timestamp, mime_type,
                file_name, md5, data)
    ...
    c.execute("INSERT INTO object_attachments (attachment_id, object_type, "
              "object_id) VALUES (%d, %s, %d)", (self.id, self.object_type,
                                                 self.object_id))
    self.save(req)
    tran.commit()
    return self
...
def item_list(req, pager, **kwargs):
    ...
    c.execute("""
        ...
        FROM attachments a
            LEFT JOIN object_attachments o
                ON (o.attachment_id = a.attachment_id)
    ...
    for row in iter(c.fetchone, None):
        ...
        item.id, item.uploader, item.timestamp, item.mime_type, \
            item.file_name, item.md5, data, item.author, item.object_type = row
